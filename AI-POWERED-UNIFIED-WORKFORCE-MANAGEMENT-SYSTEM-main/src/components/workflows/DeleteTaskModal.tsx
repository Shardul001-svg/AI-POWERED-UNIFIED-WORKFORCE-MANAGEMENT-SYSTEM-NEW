...
import { TaskEditRow } from "./EditTaskModal";
...
export function DeleteTaskModal({ isOpen, task, onClose, onSuccess }: Readonly<Props>) {
  const { t } = useI18n();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok || !payload.success) throw new Error(payload.message || "Failed to delete task");
      onSuccess(task.id); onClose();
    } catch (err: unknown) {
